...
class PosPaymentMethod(models.Model):
    ...
    def _get_stripe_secret_key(self):
        ...
        stripes = self.env['payment.acquirer'].search([('provider', '=', 'stripe')])
        for i in stripes:
            if i.stripe_secret_key != False and i.stripe_secret_key != 'False':
                stripe_secret_key = i.stripe_secret_key
                break
        if not stripes:
            raise ValidationError(_('Complete the Stripe onboarding.'))
        return stripe_secret_key
